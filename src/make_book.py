...
put_yo: bool = True
speaker: str = 'xenia'
sample_rate: int = 24000  # Hz - 48000, 24000 or 8000
torch_device: str = 'auto'  # cpu, cuda or auto
torch_num_threads: int = 2  # Only effective for torch_device = 'cpu' - use 4-6 threads, larger count may slow down TTS
...
wave_file_size_limit: int = 128 * 1024 * 1024  # 512 MiB - not more than 4GiB!
...
#    torch.backends.nnpack.enabled = False
    ...
    line_length_limit: int = line_length_limits[speaker] - 100  # Max text length for speaker
    # del origin_lines
    print(f"line_length_limit={line_length_limit}")
    preprocessed_lines, preprocessed_text_len = preprocess_text(origin_lines, line_length_limit)
    print(f"preprocessed_text_len={preprocessed_text_len}")
...
def preprocess_text(lines: list, length_limit: int) -> (list, int):
    ...
    prefix = '<speak><prosody rate="slow" pitch="medium">'
    ...
    for line in lines:
        ...
        line = re.sub(r"д.\s*н.\s*э.", " до нашей эры", line)
        line = re.sub(r"н.\s*э.", " нашей эры", line)
        ...
        while len(line) > 0:
            ...
            if len(line) < length_limit:
                ...
                line = line + "\n"
                preprocessed_lines.append(prefix + line + suffix)
#                preprocessed_lines.append(line)
            ...
            preprocessed_lines.append(prefix + part + suffix)
...
def process_tts(tts_model: torch.nn.Module, lines: list, output_filename: str, wave_data_limit: int,
    ...
    for line in lines:
        ...
        try:
            audio = tts_model.apply_tts(ssml_text=line,
                                        speaker=speaker,
